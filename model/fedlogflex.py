...
class FedLog(Model):
    ...
    def __init__(self, params):
        self.client_models = []
        n = len(params.model)
        bound = int(params.num_clients/n)
        for i in range(params.num_clients):
            self.client_models.append(params.model[int(i/bound)]())
        self.m = params.m+1 if self.client_models[0].augment else params.m
    ...
    def pretrain(self, dataset, train_index, vali_index):
        ...
        self.global_eta = torch.randn(self.m, self.num_class).to(device)
