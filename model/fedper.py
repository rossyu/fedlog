...
class FedPer(Model):
    class Parameters():
        def __init__(self, local_model, global_model, num_class, num_clients, lr, train_batch_size, test_batch_size, local_epochs, seed=None, mean_err=True):
    ...
    def local_update(self, dataset, train_index):
        ...
        for i, model in enumerate(self.client_models):
            ...
            optimizer = torch.optim.Adam(list(model[0].parameters())+list(model[1].parameters()), lr=self.lr)
